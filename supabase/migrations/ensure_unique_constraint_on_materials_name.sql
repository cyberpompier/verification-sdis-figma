/*
      # Assurer la contrainte UNIQUE sur la colonne 'name' de la table 'materials'

      1. Modifications de la table
        - `materials`
          - Ajout d'une contrainte UNIQUE sur la colonne `name` si elle n'existe pas déjà.
      2. Données
        - Ré-insertion des données d'exemple dans la table `materials` en utilisant `ON CONFLICT (name)`.
    */

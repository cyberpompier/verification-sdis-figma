/*
  # Assurer le schéma et les données de la table vehicles

  1. Modifications du schéma
    - S'assurer que la table `vehicles` existe avec toutes ses colonnes.
    - Ajouter la colonne `image_src` si elle n'existe pas, avec `text NOT NULL DEFAULT ''`.
    - Ajouter la colonne `title` si elle n'existe pas, avec `text NOT NULL DEFAULT ''`.
    - Ajouter la colonne `description` si elle n'existe pas, avec `text NOT NULL DEFAULT ''`.
    - Ajouter la colonne `last_verification` si elle n'existe pas, avec `text NOT NULL DEFAULT ''`.
  2. Sécurité
  ...
  3. Données
    - Insérer les données initiales des véhicules si elles ne sont pas déjà présentes.
*/
